...
export function createModeSelector(config: ModeSelectorConfig): ModeSelectorResult {
  ...
  // Dropdown background
  const dropdownBg = scene.add.rectangle(
    0,
    (modes.length * optionHeight) / 2,
    buttonWidth,
    modes.length * optionHeight + 8,
    0x222233,
  );
  ...
  scene.input.on(
    ...
    (_pointer: Phaser.Input.Pointer, currentlyOver: Phaser.GameObjects.GameObject[]) => {
      if (
        isOpen &&
        !currentlyOver.some((obj) => obj === buttonBg || dropdownContainer.list.includes(obj))
      ) {
        closeDropdown();
      }
    },
