...
import {
  ...
  CHEST_ORDER,
  rollChestRarity,
  rollChestCurrencyRewards,
  ChestCurrencyRewards,
} from "../data/chestData";
...
export default class ChestScene extends Phaser.Scene {
  ...
  private playChestOpenAnimation(
    chestType: ChestType,
    equipment: Equipment,
    currencyRewards: ChestCurrencyRewards,
  ): void {
    const { width, height } = this.cameras.main;
    const config = CHEST_CONFIGS[chestType];
  ...
  private showEquipmentReveal(
    ...
    statsEntries.forEach(([stat, value]) => {
      ...
      const statText = this.add
        .text(
          0,
          statsStartY + statsOffset,
          `${statName}: ${sign}${this.formatStatValue(numValue, stat)}`,
          {
            fontSize: "15px",
  ...
  private showBulkReveal(
    ...
    maskGraphics.fillRect(width / 2 - gridWidth / 2, height / 2 + gridTop, gridWidth, gridHeight);
