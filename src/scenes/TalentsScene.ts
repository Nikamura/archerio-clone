...
import { TalentId, TalentTier, Talent, TALENTS, TIER_NAMES } from "../config/talentData";
...
export default class TalentsScene extends Phaser.Scene {
  ...
  private createTalentGrid(width: number, _height: number) {
    ...
    tiers.forEach((tier) => {
      ...
      talentsByTier[tier].forEach((talent) => {
        const card = this.createTalentCard(cardX, currentY, cardWidth, cardHeight, talent);
  ...
  private playSpinAnimation(actualResult: Talent, onComplete: () => void) {
    ...
    const skipHint = this.add
      .text(width / 2, height - 80, "Tap to speed up", {
        fontSize: "14px",
        color: "#888888",
      })
      .setOrigin(0.5)
    ...
    maskShape.fillRect(
      width / 2 - frameWidth / 2 + 10,
      centerY - frameHeight / 2 + 5,
      frameWidth - 20,
      frameHeight - 10,
    );
    ...
    reelTween = this.tweens.add({
      ...
      onUpdate: (_tween, target) => {
        ...
        if (
          currentIndex !== this.lastTickIndex &&
          currentIndex >= 0 &&
  ...
  private updateUI() {
    ...
    const suffixes = ["", "", "%", "%", "%", "%"];
    const labels = ["HP", "Attack", "Dmg Reduction", "Atk Speed", "Crit Chance", "Equip Bonus"];
