...
export default class StartingAbilityScene extends Phaser.Scene {
  ...
  create() {
    try {
      ...
      const titleText = this.totalSelections > 1
        ? `STARTING ABILITY ${this.currentSelection}/${this.totalSelections}`
        : 'STARTING ABILITY'
      const title = this.add.text(0, -modalHeight / 2 + 30, titleText, {
        fontSize: '20px',
        color: '#bb88ff',
        fontStyle: 'bold',
      }).setOrigin(0.5)
      ...
      // Subtitle
      const subtitle = this.add.text(0, -modalHeight / 2 + 55, 'Glory talent bonus - Choose wisely!', {
        fontSize: '12px',
        color: '#888888',
      }).setOrigin(0.5)
      ...
      console.log(`StartingAbilityScene: Created (selection ${this.currentSelection}/${this.totalSelections})`)
    } catch (error) {
