...
export default class HeroesScene extends Phaser.Scene {
  ...
  private createHeroCards(width: number, _height: number): void {
    ...
    heroIds.forEach((heroId, index) => {
      ...
      const card = this.createHeroCard(width / 2, yPos, heroState, isSelected);
  ...
  private createButton(
    ...
    // Hover effects
    bg.on("pointerover", () => {
      bg.setFillStyle(Phaser.Display.Color.HexStringToColor(color).lighten(20).color);
    });
  ...
  private showStatsPanel(): void {
    ...
    const title = this.add
      .text(0, -panelHeight / 2 + 20, "STATS SUMMARY", {
        fontSize: "18px",
        color: "#ffffff",
        fontStyle: "bold",
      })
    ...
    const subtitle = this.add
      .text(0, -panelHeight / 2 + 40, "Base stats before chapter/weapon modifiers", {
        fontSize: "10px",
        color: "#888888",
      })
      .setOrigin(0.5);
    ...
    y = this.addStatSection(y + 10, panelWidth, "DEFENSE", [
      ...
      },
      {
        ...
        talent: 0,
        percent:
          (equipStats.damageReductionPercent ?? 0) + talentBonuses.percentDamageReduction / 100,
      },
      {
        label: "Dodge Chance",
        hero: 0,
    ...
    if (talentBonuses.percentEquipmentStats > 0) {
      const noteText = this.add
        .text(
          0,
          y + 25,
          `* Equipment stats boosted by ${talentBonuses.percentEquipmentStats}% from talents`,
          {
            fontSize: "10px",
            color: "#88cc88",
          },
    ...
    const closeHint = this.add
      .text(0, panelHeight / 2 - 25, "TAP TO CLOSE", {
        fontSize: "11px",
        color: "#888888",
      })
      .setOrigin(0.5);
  ...
  private addStatSection(
    ...
    for (const stat of stats) {
      ...
      const totalText = this.add
        .text(leftX + 100, y, `${totalStr}${suffix}`, {
          fontSize: "11px",
          color: "#ffffff",
          fontStyle: "bold",
        })
      ...
      const breakdownText = this.add
        .text(leftX + 150, y, breakdownStr + (stat.note ?? ""), {
          fontSize: "9px",
          color: "#666666",
        })
        .setOrigin(0, 0.5);
