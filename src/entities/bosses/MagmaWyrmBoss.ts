  ...
  protected selectAttackPhase(pattern: number, playerX: number, playerY: number): void {
    switch (pattern) {
      ...
      case 1:
        ...
        this.breathStartAngle =
          Phaser.Math.Angle.Between(this.x, this.y, playerX, playerY) - this.breathSweepRange / 2;
  ...
  private handleFireBreathPhase(
    ...
    if (sweepElapsed < sweepDuration) {
      ...
      while (this.breathProjectileCount < expectedProjectiles) {
        const currentAngle =
          this.breathStartAngle + (this.breathProjectileCount / 30) * this.breathSweepRange;
  ...
  private handleBodySlamPhase(time: number, elapsed: number): void {
    ...
    if (slamElapsed < slamDuration) {
      ...
      if (slamElapsed % 50 < 20) {
        ...
        const trailAngle = Phaser.Math.Angle.Between(
          this.slamEndX,
          this.slamEndY,
          this.slamStartX,
          this.slamStartY,
