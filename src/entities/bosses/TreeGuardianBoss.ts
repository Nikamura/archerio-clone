...
export default class TreeGuardianBoss extends BaseBoss {
  ...
  private handleVineWhipPhase(elapsed: number, playerX: number, playerY: number, time: number): void {
    ...
    if (elapsed < windupDuration) {
      ...
      this.clearTint()
      const angle = Phaser.Math.Angle.Between(this.x, this.y, this.vineWhipTarget.x, this.vineWhipTarget.y)
      this.clearTelegraphs()
  ...
  private handleRootTrapPhase(elapsed: number, playerX: number, playerY: number, time: number): void {
    this.setVelocity(0, 0)
  ...
  private handleLeafStormPhase(elapsed: number, time: number): void {
    ...
    while (this.leafStormProjectiles < expectedProjectiles && this.leafStormProjectiles < this.leafStormMaxProjectiles) {
      const speed = 120
