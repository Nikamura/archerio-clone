...
export default class LavaGolemBoss extends BaseBoss {
  ...
  private handleFireWavePhase(time: number, elapsed: number): void {
    ...
    if (
      elapsed % 100 < 20 &&
      this.fireWaveRadius > 20 &&
      this.fireWaveRadius < this.fireWaveMaxRadius
    ) {
      ...
      if (distToPlayer < this.lavaPoolRadius) {
        ...
        if (Math.random() < 0.02) {
          ...
          this.bulletPool.spawn(
            pool.x + Math.cos(angle) * 10,
            pool.y + Math.sin(angle) * 10,
            Phaser.Math.Angle.Between(pool.x, pool.y, playerX, playerY),
            50,
          );
        }
      }
