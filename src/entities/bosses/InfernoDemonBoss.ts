...
export default class InfernoDemonBoss extends BaseBoss {
  ...
  private handleFlamePillarsPhase(time: number, elapsed: number, playerX: number, playerY: number): void {
  ...
  private handleTeleportDashPhase(time: number, elapsed: number, playerX: number, playerY: number): void {
    // Fade out phase
    ...
    if (elapsed >= 600 && !this.hasDashed) {
      ...
      this.setVelocity(
        Math.cos(dashAngle) * 500,
        Math.sin(dashAngle) * 500
      )
  ...
  private handleEnragedComboPhase(time: number, elapsed: number, playerX: number, playerY: number): void {
    // Rapid combo of attacks during enrage
    const comboInterval = 500

    if (this.comboCount < 4) {
      if (elapsed >= this.comboCount * comboInterval && elapsed < (this.comboCount + 1) * comboInterval) {
