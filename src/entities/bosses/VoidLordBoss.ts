...
export default class VoidLordBoss extends BaseBoss {
  ...
  private handleDarknessZonesPhase(
    time: number,
    elapsed: number,
    playerX: number,
    playerY: number,
  ...
  private updateDarknessZones(time: number, playerX: number, playerY: number): void {
    for (let i = this.darknessZones.length - 1; i >= 0; i--) {
      ...
      for (let j = 0; j < 3; j++) {
        ...
        zone.graphics.arc(zone.x, zone.y, swirlRadius, swirlAngle, swirlAngle + 0.5);
      ...
      if (distToPlayer < zone.radius && Math.random() < 0.03) {
        ...
        this.bulletPool.spawn(
          zone.x,
          zone.y,
          Phaser.Math.Angle.Between(zone.x, zone.y, playerX, playerY),
          80,
        );
      }
    }
    ...
    for (const tentacle of this.tentacles) {
      ...
      // Strike phase
        ...
        const strikeProgress =
          (tentacleElapsed - this.tentacleWarningTime) / this.tentacleStrikeTime;
      ...
      // Retract phase
        ...
        const retractProgress =
          (tentacleElapsed - this.tentacleWarningTime - this.tentacleStrikeTime) / 300;
