...
export default class CrystalGuardianBoss extends BaseBoss {
  ...
  takeDamage(amount: number): boolean {
    ...
    if (!result) {
      ...
      const currentThreshold =
        Math.floor(currentPercent / this.shatterThreshold) * this.shatterThreshold;
  ...
  private createTurretDestroyEffect(x: number, y: number): void {
    ...
    for (let i = 0; i < 6; i++) {
      ...
      const shard = this.scene.add.triangle(x, y, 0, 0, 8, 0, 4, 12, 0xe0ffff, 0.8);
      shard.setDepth(5);
  ...
  private createFragment(angle: number): void {
    // Small crystal shard
    const fragment = this.scene.add.polygon(
      this.x,
      this.y,
      [
        { x: 0, y: -10 },
